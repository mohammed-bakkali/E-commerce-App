...
import { faHeart as faHeartOutline, faUser as faUserOutline } from "@fortawesome/free-regular-svg-icons";

// ── Inline styles (replaces NavbarLogin.css) ────────────────────────────────
const styles = `
  ...
  :root {
    ...
    --shadow-md: 0 4px 16px rgba(26,23,20,.10), 0 1px 4px rgba(26,23,20,.06);
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nb-icon-btn svg { font-size: 18px; }
  .nb-icon-label {
    font-size: 10px;
    font-weight: 500;
    letter-spacing: .04em;
  ...
  .nb-dropdown-menu .logout-item { color: var(--clr-red); }
  .nb-dropdown-menu .logout-item svg { color: var(--clr-red); }
  .nb-dropdown-menu .logout-item:hover { background: #fff5f5; color: var(--clr-red); }
...
const NavbarLogin = () => {
  // --- hook stubs (replace with your real hooks) ---
  const word = (typeof localStorage !== "undefined" && localStorage.getItem("searchword")) || "";
  const OnChangeSearch = (e) => {
    if (typeof localStorage !== "undefined") localStorage.setItem("searchword", e.target.value);
  };
  const itemsNum = 3; // replace with useGetAllUserCartHook().itemsNum

  // Demo user — replace with: let user = JSON.parse(localStorage.getItem("user")) || null;
  ...
  const avatarUrl = "https://i.pravatar.cc/100?img=47"; // replace with your avatar import
              ...
              <div className="nb-avatar-wrap" ref={dropdownRef}>
                ...
                <div className={`nb-dropdown ${isDropdownOpen ? "open" : ""}`}>
                  ...
                  <ul className="nb-dropdown-menu">
                    <li>
                      <FontAwesomeIcon icon={user.role === "admin" ? faTachometerAlt : faUser} />
                      {user.role === "admin" ? (
                    ...
                    <li className="nb-dropdown-divider" style={{ padding: 0 }} />
